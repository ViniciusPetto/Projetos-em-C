...
int main() {
	...
	for(i=0; i<2; i++){
		...
		printf("Digite os integrantes da banda %d: ",i+1);
		scanf("%d", &vet_banda[i].integrantes);
		
		printf("Digite a quantidade de seguidores no Twitter da banda %d: ",i+1);
	...
	printf("Digite o genero que deseja encontrar a banda: ");
	...
	for(i=0; i<2; i++){
		if(max_engajamento == vet_banda[i].engajamento){
			printf("\nBanda com maior engajamento: %sEngajamento: %d", vet_banda[i].nome, vet_banda[i].engajamento);
		}
	}
